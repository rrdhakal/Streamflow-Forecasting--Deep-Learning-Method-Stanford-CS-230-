...
class GRU_JF(nn.Module):
    ...
    def __init__(self,
        ...
        self.weight_ih = nn.Parameter(torch.FloatTensor(input_size_dyn, 2 * hidden_size))
        self.weight_hh = nn.Parameter(torch.FloatTensor(hidden_size, 2 * hidden_size))
        self.weight_sh = nn.Parameter(torch.FloatTensor(input_size_stat, hidden_size))
        self.bias = nn.Parameter(torch.FloatTensor(2 * hidden_size))
    ...
    def reset_parameters(self):
        ...
        weight_hh_data = weight_hh_data.repeat(1, 2)
    ...
    def forward(self, x_d: torch.Tensor, x_s: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for t in range(seq_len):
            ...
            #tmp = torch.ones(1)-torch.sigmoid(r)
            sigR = torch.sigmoid(r)

            #c_1 =  tmp * c_0 + torch.sigmoid(r) * i * torch.tanh(g)
            c_1 =  (1 - sigR) * c_0 + sigR * i * torch.tanh(g)
            h_1 = c_1
